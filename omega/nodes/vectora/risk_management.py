"""
omega.nodes.vectora.risk_management
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
logger = logging.getLogger("omega.nodes.vectora.risk_management")
...
class RiskManagementNode(Node):
    ...
    def improve(self, feedback: Dict[str, Any]) -> bool:
        ...
        if self._use_correlation and not self._use_regime and (
            self._last_portfolio_var > self._max_portfolio_var * 1.5 or iteration >= 3
        ):
    ...
    def _check_risk_limits(
        ...
        var_result = self._compute_portfolio_var(
            {"weights": adjusted_weights}, market_data
        )
        ...
        if self._use_correlation and len(weights) >= 2:
            ...
            if max_corr > 0.9:
                violations.append(
                    f"High pairwise correlation detected: {max_corr:.2f}"
                )
