"""
omega.nodes.victoria.verification
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
Verification and convergence monitoring nodes for the Victoria self-improvement loop.
...
from omega.nodes.victoria.cleaners import CleanerNode
...
class VerificationNode(CleanerNode):
    ...
    Called from the improvement loop in victoria_main.py.
    ...
    MAXIMIZE_METRICS: frozenset[str] = frozenset(
        {"coverage_rate", "signal_coverage", "sharpe_ratio", "completeness_score"}
    )
    ...
    def verify_improvement(
        ...
        for metric, before_val in before_metrics.items():
            ...
            elif metric in self.MINIMIZE_METRICS and after_val > before_val * (
                1 + self.REGRESSION_THRESHOLD
            ):
                regressions.append(
                    {
                        "metric": metric,
...
class InvariantDiscoveryNode(Node):
    ...
    def execute(self, input: NodeInput) -> NodeOutput:
        ...
        try:
            if input.action == "observe":
                ...
                confirmed_keys = {
                    (inv["metric"], inv["type"]) for inv in self._confirmed_invariants
                }
                for candidate in candidates:
                    ...
                    if key not in confirmed_keys:
                        ...
                        proposed_keys = {
                            (inv["metric"], inv["type"]) for inv in self._proposed_invariants
                        }
    ...
    def _record_observation(self, metrics: dict[str, float]) -> None:
        ...
        for inv in self._confirmed_invariants:
            ...
            inv_type = inv["type"]
            if (inv_type == "lower_bound" and val < inv["bound"]) or (
                inv_type == "upper_bound" and val > inv["bound"]
            ):
...
class ConvergenceMonitorNode(CleanerNode):
    ...
    def get_state(self) -> NodeState:
        slope = (
            self._linear_regression_slope(self._scores[-10:]) if len(self._scores) >= 2 else 0.0
        )
