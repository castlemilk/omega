...
class Orchestrator:
    ...
    def execute_goal(
        ...
        for task_spec in tasks:
            ...
            if node is None:
                logger.warning("No healthy node found for action '%s' — skipping", action)
                out = NodeOutput(
                    success=False,
    ...
    def evaluate_performance(
        ...
        latencies = [o.metrics.get("latency_ms", 0.0) for o in outputs if o.success]
        successes = [o for o in outputs if o.success]
        accuracy_vals = [o.metrics.get("accuracy", 1.0) for o in successes]
        ...
        for node in self.registry.all_nodes():
            ...
            try:
                ...
                if changed:
                    ...
                    logger.info("[iter %d] Node '%s' applied improvement", iteration, state.name)
                else:
                    logger.debug("[iter %d] Node '%s' declined improvement", iteration, state.name)
    ...
    def run_convergence_loop(
        ...
        """
        logger.info("Starting convergence loop: goal='%s' max_iter=%d", goal, max_iterations)
        parameters = parameters or {}
