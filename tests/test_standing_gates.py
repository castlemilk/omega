...
def test_standing_baseline_carries_the_journal_floors(config: dict):
    """crisis +$599 / trend +$2,997 / recent +$30 — training_log/V271.md:6."""
    assert config["families"]["crisis"]["pnl_floor_usd"] == 599.0
    assert config["families"]["trend"]["pnl_floor_usd"] == 2997.0
    assert config["families"]["recent"]["pnl_floor_usd"] == 30.0
...
def test_crisis_floor_passes_on_exact_equality(tmp_path: Path, config: dict):
    res = _results(tmp_path, "v300_crisis_r1", pnl=599.0, trades=42)
    trades = _trades(tmp_path, "v300_crisis_r1", _TWO_TRADES)
    out = check_standing_gates(res, trades, config)
    assert out.verdict == VERDICT_PASS
    assert out.family == "crisis"
    g = out.gates["family_pnl_floor"]
    assert g.status == STATUS_PASS
    assert g.detail["floor_usd"] == 599.0
    assert g.detail["margin_usd"] == 0.0


def test_crisis_floor_fails_one_cent_below(tmp_path: Path, config: dict):
    res = _results(tmp_path, "v300_crisis_r1", pnl=598.99, trades=42)
    trades = _trades(tmp_path, "v300_crisis_r1", _TWO_TRADES)
    out = check_standing_gates(res, trades, config)
    assert out.verdict == VERDICT_FAIL
    assert out.passed is False
    assert out.gates["family_pnl_floor"].status == STATUS_FAIL
    assert out.gates["family_pnl_floor"].detail["margin_usd"] == pytest.approx(-0.01)
    assert any("family_pnl_floor[crisis]" in f for f in out.failures)


def test_trend_floor_is_2997(tmp_path: Path, config: dict):
    res = _results(tmp_path, "v300_trend_r1", pnl=2996.0, trades=42)
    out = check_standing_gates(res, _trades(tmp_path, "v300_trend_r1", _TWO_TRADES), config)
    assert out.verdict == VERDICT_FAIL
    assert out.gates["family_pnl_floor"].detail["floor_usd"] == 2997.0


def test_recent_floor_is_30(tmp_path: Path, config: dict):
    ...
    assert out.verdict == VERDICT_PASS
    assert out.gates["family_pnl_floor"].detail["floor_usd"] == 30.0
...
def test_unmatched_family_is_no_baseline_not_a_pass(tmp_path: Path, config: dict):
    ...
    assert out.gates["family_pnl_floor"].status == STATUS_NOT_EVALUATED
...
def test_absent_trades_block_is_not_evaluated_never_pass(tmp_path: Path, config: dict):
    ...
    out = check_standing_gates(res, _trades(tmp_path, "v300_crisis_r1", _TWO_TRADES), config)
    for name in ("family_pnl_floor", "trade_count_floor", "drawdown_ceiling"):
...
def test_no_op_never_masks_a_real_failure(tmp_path: Path, config: dict):
    ...
    assert out.verdict == VERDICT_FAIL  # $10 is below the crisis floor of $599
...
def test_emitted_file_shape(tmp_path: Path, config: dict):
    ...
    assert set(payload["gates"]) == {"family_pnl_floor", "trade_count_floor", "drawdown_ceiling"}
    for gate in payload["gates"].values():
        assert gate["status"] in {STATUS_PASS, STATUS_FAIL, STATUS_NOT_EVALUATED}
    assert payload["standing_baseline_used"]["pnl_floor_usd"] == 599.0
...
def test_old_vacuous_self_comparison_no_longer_reads_as_a_bare_pass(tmp_path: Path, config: dict):
    ...
    or a real FAIL against the family floor — never a bare PASS whose only
    ...
    # $1,149.76 clears the crisis floor of $599, so v49 logic would say PASS.
    assert out.gates["family_pnl_floor"].status == STATUS_PASS
...
def test_real_corpus_gate_files_are_reported_against_the_standing_floor(tmp_path: Path, config: dict):
    ...
    crisis floor. The point of the assertion is the FAMILY: the label carries a
    date, so v49's sibling lookup resolved nothing at all and wrote no file.
    """
    ...
    assert out.gates["family_pnl_floor"].detail["floor_usd"] == 599.0
