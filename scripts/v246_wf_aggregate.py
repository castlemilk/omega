#!/usr/bin/env python3
"""
V246 — exit-adapt walk-forward aggregator.

Joins the V246 grid's `exit_adapt` cells
(v246wf_<window>_exit_adapt_<gate>_determinism/summary.json) against the
...
spot-check), and applies the pre-registered V246 falsifier
(training_log/V246.md):
...
Exit codes: 0 = ok, 5 = any exit_adapt cell's determinism verdict FAIL.
"""
...
TREATMENT_PREFIX, TREATMENT_CFG = "v246wf", "exit_adapt"
...
def main() -> int:
    ...
    if not complete or det_fail:
        ...
        verdict = "ADOPT — exit_adapt default-ON"
    else:
    ...
    md = [
        ...
        "",
        "`exit_adapt` (selective universe + exit_adaptivity: "
        "trail_keep_frac=0.25, max_hold_win=8) vs `universe_selective` (reused V240 confirm "
    ...
    for reg in regimes:
        ...
        md += [
            "",
            "Per-window Δ (exit_adapt − selective): "
    ...
    print(
        f"v246_wf_aggregate: {len(rows)} cells, {len(missing)} missing, "
