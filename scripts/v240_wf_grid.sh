#!/bin/bash
# V240 — walk-forward selective-universe confirm grid.
#
# Confirms the Track A trade-log reconstruction (scripts/v240_universe_forensics.py)
# with real runs: 32 windows x ONE config (`universe_selective`) = 32 cells.
# The baseline arm is NOT re-run — `universe_legacy` cells are reused from the
# V239 grid ($AUDIT/v239wf_*_universe_legacy_* — certified deterministic and
# byte-identical to V238 main). Aggregation joins selective vs those legacy
# cells (scripts/v240_wf_aggregate.py).
#
#   universe_selective — standing main config (V227 skew ON + regime-gated +
#                        X=0.12; IC OFF; brake OFF; predemean OFF; throttle
#                        OFF; frozen_series OFF) + universe_selective_enabled
#                        => effective blacklist {BTC, DOT, LINK}; universe =
#                        ETH/ADA/NEAR/ARB + SOL/BNB/AVAX/XRP/SUI/MATIC.
#
# Acceptance (V240.md Track A): adopt selective iff pooled mean-D > -$300 AND
# no regime mean-D < -$500 (the V239 pre-reg bar), measured full-run, not
# reconstruction. Reconstruction prediction: pooled +$512, crisis -$211,
# trend +$1,250, recent +$643.
#
...
#   nohup bash scripts/v240_wf_grid.sh > $OMEGA_AUDIT_OUTPUT_DIR/v240_wf_grid.log 2>&1 &
# Smoke (one window):
#   WINDOWS="snap_wf_20250305" bash scripts/v240_wf_grid.sh
set -u
cd "$(dirname "$0")/.."
...
OUT="$AUDIT_DIR/v240_wf"; mkdir -p "$OUT"
...
CONFIGS="${CONFIGS:-universe_selective}"

feats_for() { case "$1" in universe_selective) echo "$SELECTIVE" ;; *) echo "" ;; esac; }
...
cell_summary() { echo "$AUDIT_DIR/v240wf_${1}_${2}_${3}_determinism/summary.json"; }  # wid cfg gate
cell_done() {
...
json.dump({"version":"v240_wf","phase":phase,"cells_done":int(done),"cells_total":int(total),
           "max_parallel":1,"sleep":int(slp),
           "manifest_note":"resumable: re-run scripts/v240_wf_grid.sh to continue"},
...
try:
    ...
    d = {}
d["v240_walkforward"] = {"phase": phase, "cells_done": int(done), "cells_total": int(total),
...
log "=== V240 selective-universe grid start $(date -u +%FT%TZ) — $TOTAL cells (sleep=$SLEEP, sentinels N=2: $SENTINELS), $DONE0 already done (resume); TMPDIR=${TMPDIR:-<host>} AUDIT=$AUDIT_DIR ==="
...
log "=== V240 selective grid complete $(date -u +%FT%TZ) — aggregating ==="
write_manifest "$done" "$TOTAL" "aggregating"
python3 scripts/v240_wf_aggregate.py --root "$AUDIT_DIR" --manifest "$MANIFEST" \
        --out-json "$OUT/distribution.json" \
        --out-md "omega/nodes/victoria/training_log/V240_UNIVERSE_CONFIRM_RESULTS.md" | tee -a "$SUM"
AGG_RC=$?
write_manifest "$done" "$TOTAL" "complete"
update_session_state "complete" "$done" "$TOTAL"
log "=== V240 aggregation rc=$AGG_RC (0=ok, 5=determinism FAIL blocks verdict) ==="
