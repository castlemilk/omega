#!/bin/bash
# V245 — gdelt solo frozen walk-forward grid.
#
# 32 windows x ONE config (`gdelt_solo`) = 32 cells. The OFF arm is NOT
...
# scripts/v245_wf_aggregate.py. The 4-window OFF reproducibility spot-check
# (scripts/v241_baseline_spotcheck.sh, re-run 2026-07-13) certifies those
# cells still reproduce from current state.
#
#   gdelt_solo — V240 selective standing config + frozen_series_enabled with
#     frozen_series_signals="gdelt" + geopolitical_signals (V240.C wiring).
#     The other 5 feeds stay on their live paths (== baseline code path),
#     the V240.B solo pattern.
#
# Sentinels run N=2 (byte-identical no-op re-run per regime).
#
# Falsifier (training_log/V245.md): FAILS if ANY of
#   recent mean-D < +$100 AND recent p25-D < +$400; pooled p25-D < $0;
#   trend mean-D < -$300; crisis mean-D < -$300; gdelt never composite-visible.
...
#   nohup bash scripts/v245_gdelt_grid.sh > $OMEGA_AUDIT_OUTPUT_DIR/v245/grid.log 2>&1 &
set -u
cd "$(dirname "$0")/.."
...
cell_summary() { echo "$AUDIT_DIR/v245wf_${1}_${2}_${3}_determinism/summary.json"; }
cell_done() {
...
json.dump({"version":"v245_wf","phase":phase,"cells_done":int(done),"cells_total":int(total),
           "max_parallel":1,"sleep":int(slp),
           "manifest_note":"resumable: re-run scripts/v245_wf_grid.sh to continue"},
...
try:
    ...
    d = {}
d["v245_walkforward"] = {"phase": phase, "cells_done": int(done), "cells_total": int(total),
...
log "=== V245 gdelt-solo grid start $(date -u +%FT%TZ) — $TOTAL cells (sleep=$SLEEP, sentinels N=2: $SENTINELS), $DONE0 already done (resume); TMPDIR=${TMPDIR:-<host>} AUDIT=$AUDIT_DIR ==="
...
log "=== V245 grid complete $(date -u +%FT%TZ) — aggregating ==="
write_manifest "$done" "$TOTAL" "aggregating"
python3 scripts/v245_wf_aggregate.py --root "$AUDIT_DIR" --manifest "$MANIFEST" \
        --out-json "$OUT/distribution.json" \
        --out-md "omega/nodes/victoria/training_log/V245_WALKFORWARD_RESULTS.md" | tee -a "$SUM"
AGG_RC=$?
write_manifest "$done" "$TOTAL" "complete"
update_session_state "complete" "$done" "$TOTAL"
log "=== V245 aggregation rc=$AGG_RC (0=ok, 5=determinism FAIL blocks verdict) ==="
