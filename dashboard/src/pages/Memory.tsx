...
export default function Memory() {
  ...
  useEffect(() => {
    ...
    client.getNodeMemory({ nodeId: selectedId }).then((r) => {
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center gap-3">
        ...
        {selectedId && (
          <span className="text-xs text-gray-500">{counts.ep} episodes · {counts.sem} semantic concepts</span>
        )}
      </div>
      ...
      {selectedId && (
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
          <div className="bg-gray-800 rounded-xl border border-gray-700 p-4">
            <h3 className="text-sm font-semibold text-gray-400 uppercase mb-3">Semantic Concepts</h3>
            <div className="space-y-2">
              {concepts.map((c, i) => (
                <div key={i} className="flex items-start gap-2">
                  ...
                  <span className="text-xs text-indigo-400 shrink-0">{(c.confidence * 100).toFixed(0)}%</span>
                </div>
              ))}
              {concepts.length === 0 && <p className="text-gray-600 text-sm">No semantic concepts yet</p>}
            </div>
          </div>
          <div className="bg-gray-800 rounded-xl border border-gray-700 p-4">
              ...
              {episodes.map((e) => (
                <div key={e.episodeId} className="border-l-2 border-indigo-500 pl-3 py-1">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-xs text-gray-600">{(e.importance * 100).toFixed(0)}% importance</span>
                  </div>
                  {e.tags.length > 0 && (
