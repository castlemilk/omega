  ...
  return (
    <div className="bg-gray-800 rounded-lg border border-gray-700 p-3 space-y-1.5">
      <div className="flex items-center justify-between gap-2">
        ...
        {c.targetSubsystem && (
          <span className="text-xs text-gray-500 truncate max-w-[100px]">{c.targetSubsystem}</span>
        )}
      </div>
      <p className="text-xs text-gray-300 leading-relaxed">{c.description || "No description"}</p>
      <p className="text-xs text-gray-600">
        {c.createdAt ? new Date(Number(c.createdAt.seconds) * 1000).toLocaleDateString() : "—"}
      </p>
    </div>
  );
}
...
export default function Challenges() {
  const fetchFn = useCallback(() => client.getChallenges({ statusFilter: "" }), []);
  const { data, error, loading } = usePolling(fetchFn);
  const challenges = data?.challenges ?? [];

  const byStatus = (status: string) => challenges.filter((c) => c.status === status);

  if (loading) return <div className="text-gray-500 text-sm">Loading challenges…</div>;
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center gap-2">
        ...
        <h1 className="text-lg font-semibold text-white">Devil's Advocate Challenges</h1>
        <span className="text-xs text-gray-500 ml-1">{challenges.length} total</span>
      </div>
              ...
              >
                ...
                <div className="p-3 space-y-2 flex-1">
                  {col.length === 0 ? (
                    ...
                    col.map((c) => <ChallengeCard key={c.challengeId} c={c} />)
                  )}
                </div>
              </div>
            );
