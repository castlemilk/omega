#!/bin/bash
# V207a audit driver — 3 gates × 2 pairs, sequential.
set -u
cd "$(dirname "$0")/../.."
ROOT=$(pwd)
AUDIT="$ROOT/data/v207a_audit"
LOG="$AUDIT/run.log"
...
echo "=== V207a audit starting at $(date -u +%FT%TZ) ===" | tee -a "$LOG"
...
for gate in recent trend crisis; do
  for run in r1 r2; do
    ver="v207a_${gate}_${run}"
    outdir="$AUDIT/${gate}_${run}"
    mkdir -p "$outdir"
    snap=$(snap_for "$gate")
    echo "--- $ver starting $(date -u +%FT%TZ) (snap=$snap) ---" | tee -a "$LOG"
...
echo "=== V207a audit finished at $(date -u +%FT%TZ) ===" | tee -a "$LOG"
